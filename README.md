...
| ---------------------------------------------------------------- | ------------------------------- | ---------------------------------------------------------------------------------- | --------------------------------- |
| Recebe qualquer dado de request, header, cookie, fila ou webhook | Entrada sem contrato            | Existe schema (Pydantic, zod, struct) que rejeita campo desconhecido?              | Validação de entrada              |
| Monta uma query SQL usando valor que veio de request             | SQL Injection                   | O valor entra por placeholder (`?` ou `%s`) e não por string?                      | SQL Injection                     |
| Deixa o usuário escolher coluna, tabela ou ordem do resultado    | SQL Injection                   | O valor recebido é comparado com uma lista fixa no código?                         | SQL Injection                     |
| Abre, lê, grava ou envia um arquivo cujo nome veio de fora       | Path Traversal                  | O caminho final é resolvido e conferido contra a pasta base?                       | Path Traversal                    |
| Recebe upload                                                    | Path Traversal                  | O arquivo é gravado com nome gerado pela aplicação, fora da raiz web?              | Path Traversal                    |
| Cria rota, endpoint ou handler novo                              | Rota sem auth                   | A rota está coberta pelo middleware global, ou está na lista pública de propósito? | Rota sem autenticação             |
| Escreve serviço interno que outro serviço chama                  | Rota sem auth                   | Ele exige credencial de serviço mesmo sem porta publicada?                         | Rota sem autenticação, Containers |
| Busca, altera ou apaga um registro por id                        | IDOR                            | A query filtra também por `owner_id` ou `org_id` do usuário logado?                | IDOR                              |
| Recebe JSON e grava direto no banco (create ou update)           | IDOR (mass assignment)          | Existe schema com só os campos que o usuário pode enviar?                          | IDOR                              |
| Chama um binário, script ou comando do sistema                   | Command Injection               | Argumentos em lista, regex fechada, `timeout`, sem `shell=True`?                   | Command Injection                 |
| Faz requisição HTTP para URL que veio do usuário                 | SSRF                            | Host em allowlist, ou esquema e IP validados e redirect desligado?                 | SSRF                              |
| Renderiza HTML, markdown ou texto rico vindo do usuário no React | XSS                             | Passou por DOMPurify antes do `dangerouslySetInnerHTML`?                           | XSS no front                      |
| Coloca URL vinda do usuário em `href`, `src` ou redirect         | XSS                             | O esquema foi validado como `http` ou `https`?                                     | XSS no front                      |
| Monta HTML ou e-mail em Go com template                          | XSS                             | É `html/template` e não `text/template`?                                           | XSS no front                      |
| Escreve ou altera Dockerfile ou compose                          | Container mal configurado       | Tem `USER` não root, sem segredo na imagem, `ports:` só no que é público?          | Containers                        |
| Precisa de senha, chave ou token dentro da aplicação             | Segredo exposto                 | Vem de variável de ambiente ou secret, e não do código ou da imagem?               | Segredos e configuração           |
| Trata erro ou exceção que volta para o cliente                   | Erro verboso                    | O cliente recebe mensagem genérica e o detalhe fica só no log?                     | Segredos e configuração           |
| Grava ou confere senha de usuário                                | Hash fraco                      | Usa bcrypt ou argon2 da biblioteca padrão?                                         | Segredos e configuração           |
| Adiciona ou atualiza dependência                                 | Pacote vulnerável               | Lockfile atualizado e audit do CI sem severidade alta?                             | Segredos e configuração           |
| Escreve log                                                      | Vazamento em log                | O log não grava senha, token, cookie ou documento?                                 | Segredos e configuração           |
| Escreve handler de login, de erro 401/403/404 ou de validação    | Ataque invisível para o SOC     | Emite evento JSON nomeado com os campos base?                                      | O que logar para o SOC            |
| Vai abrir o PR                                                   | Falha que o revisor não vai ver | Rodou os três curls e escreveu os testes 401 e 404?                                | Como testar você mesmo            |
| Não sabe se a falha encontrada trava o merge                     | Prioridade errada               | Consultou a tabela de severidade?                                                  | Severidade                        |
...
## Validação de entrada: a base de tudo
...
## Falhas de serviços em containers
...
## O que logar para o SOC enxergar o ataque
...
## Como testar você mesmo antes do PR
...
## Severidade: o que trava o deploy e o que vira ticket
